...
class ReviewServiceApplicationTests extends PostgresTestBase {
  ...
  void getReviewsByProductId() {
    ...
    client.get().uri("/review?productId=" + productId).accept(APPLICATION_JSON)
  ...
  void getReviewsInvalidProductId() {
    ...
    client.get().uri("/review?productId=" + productId).accept(APPLICATION_JSON)
        .exchange().expectStatus().isEqualTo(UNPROCESSABLE_ENTITY)
  ...
  void getReviewsMissingParameter() {
    client.get().uri("/review").accept(APPLICATION_JSON)
        .exchange().expectStatus().isEqualTo(BAD_REQUEST)
        .expectHeader().contentType(APPLICATION_JSON)
        .expectBody().jsonPath("$.path").isEqualTo("/review")
        .jsonPath("$.message")
  ...
  void getReviewsNoneIntParm() {
    client.get().uri("/review?productId=no-intger").accept(APPLICATION_JSON)
        .exchange().expectStatus().isEqualTo(BAD_REQUEST)
        .expectHeader().contentType(APPLICATION_JSON)
        .expectBody().jsonPath("$.path").isEqualTo("/review")
